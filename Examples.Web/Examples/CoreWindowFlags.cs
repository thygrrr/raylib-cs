// Adapted for the browser from Examples/Core/WindowFlags.cs
// NOTE: The host owns the window, so the original SetConfigFlags(...) call (which only has
// an effect before InitWindow) is intentionally dropped. Most window-state operations
// (fullscreen, minimize, resize, topmost, ...) are no-ops or unreliable in the browser, but
// the state toggles still run and the bouncing ball / mouse rendering work as a demo.
using static Raylib_cs.ConfigFlags;

namespace Examples.Web;

public class CoreWindowFlags : IWebExample
{
